...
- For EBITDA_ADDBACK, extract every table row (item, counterparty, amount) including rows
  below the materiality threshold, plus the materiality floor sentence. Do not omit below-threshold rows.
...
def _serialize_adjustment(
    *,
    ...
    if kind == "EBITDA_ADDBACK":
        floor = extracted.materiality_floor
        if floor is None:
            conflicts.append(
                {
                    "kind": "FLOOR_MISSING",
        ...
        payload["materiality_floor"] = _decimal_to_str(floor) if floor is not None else None
        serialized_rows: list[dict[str, Any]] = []
        above_floor_total = Decimal("0")
        for row in extracted.ebitda_rows:
            ...
            if floor is None:
                ...
                if row.included_in_addback is not None and row.included_in_addback != above_floor:
                    conflicts.append(
                        {
                            "kind": "FLOOR_PREFILTER_MISMATCH",
                            "scenario_id": scenario_id,
                            "adjustment": adjustment_id,
                            "counterparty": row.counterparty,
                            "amount": _decimal_to_str(row.amount),
                            "model_included": row.included_in_addback,
                            "computed_above_floor": above_floor,
                        },
                    )
                if above_floor:
                    above_floor_total += row.amount
        ...
        payload["above_floor_subtotal"] = _decimal_to_str(above_floor_total)
...
async def _classify_vision_pages(
    ...
    *,
    ...
    pages: list[str],
) -> tuple[list[AdjustmentExtract], str, list[dict[str, Any]]]:
...
def _check_ebitda_invariants(
    adjustments: dict[str, dict[str, Any]],
    *,
    conflicts: list[dict[str, Any]],
    review: list[dict[str, Any]],
) -> None:
    for adjustment_id, adj in adjustments.items():
        if adj.get("kind") != "EBITDA_ADDBACK":
            continue

        floor_text = adj.get("materiality_floor")
        rows = adj.get("rows") or []
        scenario_id = adj.get("scenario_id")
        ...
        floor = Decimal(str(floor_text))
        computed_above_total = Decimal("0")
        for index, row in enumerate(rows):
            amount = Decimal(str(row["amount"]))
            expected_above = amount >= floor
            actual_above = row.get("above_floor")
            if actual_above is None:
                entry = {
                    "kind": "ROW_CLASSIFICATION_MISSING",
                    "scenario_id": scenario_id,
                    "adjustment": adjustment_id,
                    "row_index": index,
                }
                conflicts.append(entry)
                review.append(entry)
            elif actual_above != expected_above:
                entry = {
                    "kind": "FLOOR_CLASSIFICATION_MISMATCH",
                    "scenario_id": scenario_id,
                    "adjustment": adjustment_id,
                    "row_index": index,
                    "expected_above_floor": expected_above,
                    "actual_above_floor": actual_above,
                }
                conflicts.append(entry)
                review.append(entry)
            if expected_above:
                computed_above_total += amount

        stored_subtotal = Decimal(str(adj.get("above_floor_subtotal", "0")))
        if stored_subtotal != computed_above_total:
            entry = {
                "kind": "ABOVE_FLOOR_SUBTOTAL_MISMATCH",
                "scenario_id": scenario_id,
                "adjustment": adjustment_id,
                "stored": _decimal_to_str(stored_subtotal),
                "computed": _decimal_to_str(computed_above_total),
            }
            conflicts.append(entry)
            review.append(entry)

        adj["above_floor_subtotal"] = _decimal_to_str(computed_above_total)
...
def _check_payload_structure(
    ...
    *,
    ...
    if "unrecognised" not in payload:
        conflicts.append({"kind": "SCHEMA_MISSING", "field": "unrecognised"})
        review.append({"kind": "SCHEMA_MISSING", "field": "unrecognised"})
        return

    adjustments = payload.get("adjustments") or {}
    _check_ebitda_invariants(adjustments, conflicts=conflicts, review=review)
...
async def _run_async(work_dir: Path) -> StageResult:
    ...
    if is_canonical_open_dataset(work_dir):
        _check_payload_structure(payload, conflicts=conflicts, review=review)
        payload["summary"]["conflict_count"] = len(conflicts)
        payload["summary"]["review_count"] = len(review)
